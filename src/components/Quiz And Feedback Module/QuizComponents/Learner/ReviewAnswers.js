...
const ReviewAnswers = ({ attemptId }) => {
  ...
  if (!localReviewData || !Array.isArray(localReviewData.questionResponses) || localReviewData.questionResponses.length === 0) {
  ...
  return (
    <div className="review-answers-page">
        ...
        <div className="questions-container">
          {localQuestions.map((question, index) => (
            <div key={index} id={`question-${index}`} className="question-container">
              <h3 className="review-question">{index + 1}.{question.question}</h3>
              <ul className="question-options">
                {question.options.map((option, optionIndex) => {
                  const response = localReviewData.questionResponses.find(
                    q => q.quizQuestionId === question.quizQuestionId
                  );
                  const isAnswered = response?.selectedOptions?.includes(option.option) || false;
                  return (
                    <li key={optionIndex}>
      ...
      {showPopup && (
        <div className="reviewanswers-popup">
          <div className="popup-content">
            <button className="popup-close-button" style={{padding:"2px"}} onClick={() => setShowPopup(false)}>×</button>
            <br />
            <br />
            {unansweredError && (
            ...
            {!unansweredError && (
              <>
                <p className="submit-quiz-text">Are you sure you want to submit?</p>
      ...
      {submitSuccess && <div className="success-message">Submitted successfully!</div>}
    </div>
  );
};
