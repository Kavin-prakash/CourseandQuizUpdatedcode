    ...
    return (
        <Container  fluid className='mt-1' style={divStyle}>
            {loading ? (
                ...
                topicsDetail.map((topic, index) => (
                    <Card key={index} style={{ marginBottom: '20px', justifyContent: 'space-between', marginTop: '20px', fontSize: '18px' }}>
                        <Card.Header>
                            <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                        ...
                        <Card.Body>
                            <Card.Text><FaHandPointRight style={{ fontSize: '20px', color: 'gray', marginRight: '10px' }} /><b>Description :</b> {topic.topicDescription}</Card.Text>
                            <ul>
                                {topic.materials.map((material) => (
                                    <li key={material.materialId} onClick={() => handlePreview(material.filePath, material.materialType, material.materialName, material.materialId)}>
                                        {material.materialType === 'VIDEO' && <CiYoutube className="icon" style={{ color: 'blue' }} />}
                                        {material.materialType === 'AUDIO' && <CiMusicNote1 className="icon" style={{ color: 'blue' }} />}
                                        {material.materialType === 'TEXT' && <FaFileAlt className="icon" style={{ color: 'red' }} />}
                                        {material.materialType === 'PDF' && <BsFiletypePdf className="icon" style={{ color: 'red' }} />}
                                        {material.materialType === 'PPT' && <BsFiletypePpt className="icon" style={{ color: 'red' }} />}
            ...
            <Modal show={show} onHide={handleModelClose} centered size="lg" style={{ marginTop: '60px', height: '680px' }}>
                <Modal.Header closeButton>
                    <Modal.Title>{viewerModelHeader}</Modal.Title>                                                     {/*Modified lines */}
                </Modal.Header>
                <Modal.Body style={{ minHeight: "83vh" }}  >
