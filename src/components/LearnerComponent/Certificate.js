...
const Certificate = () => {
  ...
  const certificatedetails =
    enrolledCourses?.filter(
      (course) =>
        course.completedStatus === 1 && course.enrollmentid === enrollmentid
    ) || [];
  ...
  const firstname = useSelector(
    (state) => state.fetchlearner.userData.firstName
  );
  ...
  return (
    <div className="completioncertificate" ref={certificateRef}>
      ...
      <div className="content">
        ...
        <h2 id="CompletionTitle" style={{ marginLeft: "-5px" }}>
          OF COMPLETION
        </h2>
        {certificatedetails.map((course) => (
          <div>
            ...
            <h4 id="CourseName">{course.enrolledCoursename} Course</h4>
          </div>
        ))}
      ...
      <div className="download-options">
        <Button style={{ marginTop: "3%" }} onClick={handleOpenDialog}>
